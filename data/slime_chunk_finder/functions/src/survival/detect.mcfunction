execute as @e[type=item,nbt={Item:{id:"minecraft:slime_ball"}}] at @s if entity @e[distance=..1,type=item,nbt={Item:{id:"minecraft:compass"}}] run function slime_chunk_finder:src/survival/craft_slime_finder
